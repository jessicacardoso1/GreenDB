...
if [ $# -lt 2 ]; then
  ...
  echo 1>&2 "usage: bash $0 <number of iterations> <question number> <question arguments>"
...
mkdir -p results/question_$1/$now
mkdir -p graphs/question_$1
# Cria diretórios para armazenar resultados e gráficos com base no número da pergunta e na data e hora atual.
...
for i in $(seq 1 $nb_iterations); do
    ...
    filtered_csv="./results/question_"$1"/"$now"/"$i"_"$now"_joularJX-methods-energy-filtered.csv"
    graph="./graphs/question_"$1"/"$now"_methods-consumption.png"

    # Move os arquivos do JoularJX para os diretórios especificados
...
python3 jx_plot.py "results/question_"$1"/"$now $graph
